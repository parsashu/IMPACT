function PACT_NoiseRobustness()
% PACT_BENCHMARK_V90_PAPER_READY
...
% NOISE-ONLY REVISION RUNNER:
% Reuse the completed v9.3 cache and never launch a new k-Wave forward solve.
% Do not overwrite the original run manifest.
if ~isfolder(cfg.outputDir)
    error('PACT:MissingBenchmarkRoot', ...
        ['Expected existing benchmark folder was not found:\n%s\n' ...
         'Run this file from the same parent folder that contains ' ...
         'benchmark_v93_reviewer_revision.'], cfg.outputDir);
end

% The reviewer comment concerns the -5 dB full-view crossover specifically.
% Restrict reconstruction to DAS, DMAS, and DASDSF to minimize runtime while
% preserving the predeclared phantom/noise-model stress test.
cfg.noiseRobustnessAlgorithms = ["DAS","DMAS","DASDSF"];
...
cfg.noiseRobustnessModels = ["IID_GAUSSIAN", ...
    "CHANNEL_CORRELATED_GAUSSIAN","HETEROSCEDASTIC_GAUSSIAN","LAPLACE_IID"];
...
%% ========================================================================
% REVIEWER-1 NOISE-MODEL ROBUSTNESS ONLY
% ========================================================================
fprintf('\n======================================================================\n');
fprintf(' PACT v9.5 - NOISE-ROBUSTNESS-ONLY REVISION RUNNER\n');
fprintf(' Reusing cached v9.3 heterogeneous full-wave sensor data.\n');
fprintf(' No k-Wave forward simulation will be executed.\n');
fprintf(' SNR: %.1f dB | phantoms: %d | noise models: %d | realizations: %d\n', ...
    cfg.noiseRobustnessSNRdB,numel(cfg.noiseRobustnessPhantomNames), ...
    numel(cfg.noiseRobustnessModels),cfg.noiseRobustnessRealizations);
fprintf(' Algorithms: %s\n',strjoin(cellstr(cfg.noiseRobustnessAlgorithms),', '));
fprintf('======================================================================\n');

% Strict cache preflight: fail loudly instead of silently skipping a phantom.
for requestedName = cfg.noiseRobustnessPhantomNames
    cacheFile = fullfile(cfg.outputDir,char(requestedName), ...
        'cached_forward_data','clean_full_ring_data.mat');
    if ~isfile(cacheFile)
        error('PACT:MissingForwardCache', ...
            ['Missing cached forward data for %s:\n%s\n' ...
             'This noise-only runner will not launch k-Wave.'], ...
             char(requestedName),cacheFile);
    end
end
fprintf('Cache preflight: PASS\n');
...
if isempty(noiseRobustnessResults)
    error('PACT:EmptyNoiseRobustnessResults', ...
        'Noise robustness runner returned an empty table.');
end

finalCsv = fullfile(cfg.outputDir,'noise_robustness_results.csv');
finalMat = fullfile(cfg.outputDir,'noise_robustness_results.mat');
writetable(noiseRobustnessResults,finalCsv);
save(finalMat,'noiseRobustnessResults','cfg','-v7.3');

fprintf('\n======================================================================\n');
fprintf(' NOISE ROBUSTNESS COMPLETE\n');
fprintf(' Rows: %d\n',height(noiseRobustnessResults));
fprintf(' CSV : %s\n',finalCsv);
fprintf(' MAT : %s\n',finalMat);
fprintf('======================================================================\n');

% Optional immediate post-processing if the companion script is on path.
if exist('analyze_noise_robustness','file') == 2
    try
        analyze_noise_robustness(finalCsv, ...
            fullfile(pwd,'NOISE_ROBUSTNESS_REVISION'));
        fprintf('Post-processing figures/tables created in NOISE_ROBUSTNESS_REVISION.\n');
    catch ME
        warning('PACT:NoisePostprocessFailed', ...
            'Core CSV was saved, but post-processing failed: %s',ME.message);
    end
end
...
ensureDirectory(outputDir);
partialCsv = fullfile(cfg.outputDir,'noise_robustness_results_PARTIAL.csv');
partialMat = fullfile(cfg.outputDir,'noise_robustness_results_PARTIAL.mat');

% Resume a prior interrupted noise-only run if present.
if cfg.resume && isfile(partialCsv)
    results = readtable(partialCsv,'TextType','string');
    stringVars = intersect(["Phantom","NoiseModel","SensorGeometry","Algorithm"], ...
        string(results.Properties.VariableNames));
    for k = 1:numel(stringVars)
        results.(stringVars(k)) = string(results.(stringVars(k)));
    end
    fprintf('Resuming noise robustness from %d existing rows.\n',height(results));
else
...
for requestedName = cfg.noiseRobustnessPhantomNames
    localPos = find(selectedNames==requestedName,1);
    if isempty(localPos)
        error('PACT:NoisePhantomNotFound','Requested phantom %s is not available.', ...
            char(requestedName));
    end
    ...
    dataFile = fullfile(cfg.outputDir,char(phantomName), ...
        'cached_forward_data','clean_full_ring_data.mat');
    if ~isfile(dataFile)
        error('PACT:MissingForwardCache','Missing cache: %s',dataFile);
    end
    loaded = load(dataFile,'cleanFullData');
    cleanFullData = double(loaded.cleanFullData);

    fprintf('\nNoise robustness phantom: %s\n',char(phantomName));
    ...
    for geometry = cfg.noiseRobustnessGeometries
        ...
        for noiseModel = cfg.noiseRobustnessModels
            for realization = 1:cfg.noiseRobustnessRealizations
                ...
                % Resume-safe case check.
                expectedAlgorithms = cfg.noiseRobustnessAlgorithms;
                done = false(size(expectedAlgorithms));
                for aa = 1:numel(expectedAlgorithms)
                    done(aa) = any(results.Phantom==string(phantomName) & ...
                        results.NoiseModel==string(noiseModel) & ...
                        results.SensorGeometry==string(geometry) & ...
                        results.Algorithm==expectedAlgorithms(aa) & ...
                        results.NoiseRealization==realization);
                end
                if all(done)
                    continue;
                ...
                rng(caseSeed(cfg.masterSeed+820001,p,geometry, ...
                    cfg.noiseRobustnessSNRdB,realization) + ...
                    sum(double(char(noiseModel))),'twister');
                ...
                outputs = reconstructAlgorithmSet(delayed, ...
                    cfg.noiseRobustnessAlgorithms,cfg,context);

                for a = 1:numel(outputs)
                    alg = string(outputs(a).Algorithm);
                    alreadyDone = any(results.Phantom==string(phantomName) & ...
                        results.NoiseModel==string(noiseModel) & ...
                        results.SensorGeometry==string(geometry) & ...
                ...
                % Crash-safe checkpoint after every realization.
                writetable(results,partialCsv);
                save(partialMat,'results','cfg','-v7.3');

                fprintf('  %-18s | %-28s | rep %d/%d | rows %d\n', ...
                    char(geometry),char(noiseModel),realization, ...
                    cfg.noiseRobustnessRealizations,height(results));
            end
        end
    end
end

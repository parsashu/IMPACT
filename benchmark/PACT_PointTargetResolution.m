function PACT_PointTargetResolution()
% PACT_PointTargetResolution
% -------------------------------------------------------------------------
% point-target spatial-resolution completion.
%
% This runner:
%   1) reuses the existing v9.3 resolution checkpoint;
%   2) reuses cached clean point-target forward data for points 1--3;
%   3) runs NEW k-Wave forward simulations only for point locations whose
%      clean cache is absent (diagnostic showed points 4--9 are absent);
%   4) does NOT rerun completed reconstruction cases;
%   5) excludes degenerate full-ring adaptive-MV variants from the revised
%      comparison, while retaining all direct methods in full view and all
%      direct + adaptive methods in limited view;
%   6) uses the original v9.3 parameters, SNRs, realizations, random-seed
%      rule, detector model, and delay model without retuning.
%
% Revised target table size:
%   9 points x 2 SNR x 5 realizations x
%      [7 full-view direct + 11 limited-view direct/adaptive algorithms]
%   = 1620 rows.
%
% All work is written to RESOLUTION_9POINT_REVISION. The original v9.3
% checkpoint and cached files are treated as read-only.
% -------------------------------------------------------------------------
...
oldResolutionDir = fullfile(cacheRoot,'resolution_benchmark');
outputDir = fullfile(projectRoot,'RESOLUTION_9POINT_REVISION');
pointCacheDir = fullfile(outputDir,'point_forward_cache');

if ~isfolder(cacheRoot)
    error('PACT:MissingCacheRoot', ...
        'Missing benchmark folder: %s',cacheRoot);
end
if ~isfolder(oldResolutionDir)
    error('PACT:MissingResolutionFolder', ...
        'Missing original resolution folder: %s',oldResolutionDir);
end
if ~isfolder(outputDir), mkdir(outputDir); end
if ~isfolder(pointCacheDir), mkdir(pointCacheDir); end

configFile = fullfile(cacheRoot,'simulation_configuration.mat');
if ~isfile(configFile)
    error('PACT:MissingConfiguration','Missing: %s',configFile);
end
L = load(configFile,'cfg','background','sensorXFull','sensorYFull', ...
    'limitedIndices');
required = {'cfg','background','sensorXFull','sensorYFull','limitedIndices'};
for k=1:numel(required)
    if ~isfield(L,required{k})
        error('PACT:IncompleteConfiguration', ...
            'simulation_configuration.mat lacks "%s".',required{k});
    end
end

cfg = L.cfg;
background = L.background;
sensorXFull = L.sensorXFull;
sensorYFull = L.sensorYFull;
limitedIndices = L.limitedIndices;

% Keep original numerical and algorithmic settings.
...
if size(cfg.resolutionPointLocationsMM,1) ~= 9
    error('PACT:UnexpectedPointGrid', ...
        'Expected 9 configured resolution points; found %d.', ...
        size(cfg.resolutionPointLocationsMM,1));
end

fullAlgorithms = cfg.cheapAlgorithms;
limitedAlgorithms = unique([cfg.cheapAlgorithms,cfg.adaptiveAlgorithms],'stable');
snrValues = cfg.resolutionSNRdB;
numReps = cfg.resolutionRealizations;

fprintf('======================================================================\n');
fprintf(' PACT v9.7 - 9-POINT RESOLUTION REVIEWER COMPLETION\n');
fprintf('======================================================================\n');
fprintf('Original resolution folder : %s\n',oldResolutionDir);
fprintf('Revision output folder     : %s\n',outputDir);
fprintf('Points                     : %d\n',size(cfg.resolutionPointLocationsMM,1));
fprintf('SNR                         : %s\n',mat2str(snrValues));
fprintf('Realizations / SNR          : %d\n',numReps);
fprintf('Full-view algorithms        : %d (direct methods only)\n',numel(fullAlgorithms));
fprintf('Limited-view algorithms     : %d (direct + adaptive)\n',numel(limitedAlgorithms));
fprintf('Expected revised rows       : 1620\n');
fprintf('No parameter retuning will be performed.\n');
fprintf('======================================================================\n\n');

%% Rebuild the exact simulation grid and detector geometry.
...
[karray,sensorXCheck,sensorYCheck,limitedCheck] = ...
    makeFiniteApertureArrayV90(kgrid,cfg);

if max(abs(sensorXCheck(:)-sensorXFull(:))) > 1e-12 || ...
        max(abs(sensorYCheck(:)-sensorYFull(:))) > 1e-12
    error('PACT:SensorGeometryMismatch', ...
        'Reconstructed full-ring sensor coordinates do not match v9.3.');
end
if ~isequal(limitedCheck(:),limitedIndices(:))
    error('PACT:LimitedIndexMismatch', ...
        'Reconstructed limited-view indices do not match v9.3.');
end
...
%% Import the useful subset of the previous checkpoint.
oldCheckpointCsv = fullfile(oldResolutionDir,'resolution_results_checkpoint.csv');
oldFinalCsv = fullfile(cacheRoot,'resolution_results.csv');

if isfile(oldFinalCsv)
    oldResults = readtable(oldFinalCsv,'TextType','string');
    importedFrom = oldFinalCsv;
elseif isfile(oldCheckpointCsv)
    oldResults = readtable(oldCheckpointCsv,'TextType','string');
    importedFrom = oldCheckpointCsv;
else
    oldResults = emptyResolutionTable();
    importedFrom = "";
end
...
if isempty(oldResults)
    ...
    keep = false(height(oldResults),1);
    for r=1:height(oldResults)
        geom = string(oldResults.SensorGeometry(r));
        alg = string(oldResults.Algorithm(r));
        snrDb = oldResults.RequestedSNR_dB(r);
        pointIdx = oldResults.PointIndex(r);

        validPoint = pointIdx>=1 && pointIdx<=9;
        validSNR = anySNRMatch(snrDb,snrValues);
        if geom=="CIRCULAR_FULL"
            validAlg = any(alg==fullAlgorithms);
        elseif geom=="LIMITED_VIEW_ARC"
            validAlg = any(alg==limitedAlgorithms);
        else
            validAlg = false;
        end
        keep(r) = validPoint && validSNR && validAlg;
    end
    results = oldResults(keep,:);
end
results = uniqueResolutionRowsV97(results);

fprintf('Imported useful prior rows: %d\n',height(results));
if strlength(string(importedFrom))>0
    fprintf('Imported from: %s\n',importedFrom);
end

% Resume a previous v9.7 run if present.
partialCsv = fullfile(outputDir,'resolution_9point_results_PARTIAL.csv');
partialMat = fullfile(outputDir,'resolution_9point_results_PARTIAL.mat');
if isfile(partialCsv)
    newer = readtable(partialCsv,'TextType','string');
    results = [results;newer]; %#ok<AGROW>
    results = uniqueResolutionRowsV97(results);
    fprintf('After v9.7 resume merge: %d rows available.\n',height(results));
end

expectedRows = 9 * numel(snrValues) * numReps * ...
    (numel(fullAlgorithms)+numel(limitedAlgorithms));
fprintf('Rows still missing before run: %d\n\n',expectedRows-height(results));
...
for pointIndex = 1:size(cfg.resolutionPointLocationsMM,1)
    ...
    oldPointFile = fullfile(oldResolutionDir, ...
        sprintf('point_%02d_clean_data.mat',pointIndex));
    newPointFile = fullfile(pointCacheDir, ...
        sprintf('point_%02d_clean_data.mat',pointIndex));

    if isfile(newPointFile)
        tmp = load(newPointFile,'cleanFullData');
        cleanFullData = tmp.cleanFullData;
        forwardSource = "v9.7 cache";
    elseif isfile(oldPointFile)
        tmp = load(oldPointFile,'cleanFullData');
        cleanFullData = tmp.cleanFullData;
        forwardSource = "v9.3 cache";
    else
        fprintf('\nPoint %d/9 at (%.2f, %.2f) mm: running ONE new k-Wave forward simulation...\n', ...
            pointIndex,pointMM(1),pointMM(2));
        cleanFullData = single(simulateKWaveFiniteArray( ...
            p0,background,kgrid,karray,cfg));
        save(newPointFile,'cleanFullData','p0','pointMM','-v7.3');
        forwardSource = "new v9.7 k-Wave";
    end

    fprintf('\nPoint %d/9 | (%.2f, %.2f) mm | forward data: %s\n', ...
        pointIndex,pointMM(1),pointMM(2),forwardSource);
    ...
    for geometry = cfg.sensorGeometries
        ...
        if geometry=="CIRCULAR_FULL"
            algorithms = fullAlgorithms;
        else
            algorithms = limitedAlgorithms;
        end

        for snrDb = snrValues
            for realization = 1:numReps
                missingAlgorithms = strings(0,1);
                for a=1:numel(algorithms)
                    if ~resolutionRowExistsV97(results,pointIndex,geometry, ...
                            algorithms(a),snrDb,realization)
                        missingAlgorithms(end+1,1) = algorithms(a); %#ok<AGROW>
                    end
                end
                ...
                % Same resolution-benchmark seed rule as v9.3.
                rng(caseSeed(cfg.masterSeed+700001,pointIndex,geometry, ...
                    snrDb,realization),'twister');
                [noisyData,~,measuredSNR] = addNoiseAtRequestedSNR(cleanData,snrDb);
                noiseVar = estimatePretriggerNoiseVariance( ...
                ...
                outputs = reconstructAlgorithmSet( ...
                    delayed,missingAlgorithms,cfg,context);
                ...
                for a=1:numel(outputs)
                    ...
                    row(1,:) = {pointIndex,pointMM(1),pointMM(2), ...
                        string(geometry),algorithmName,snrDb,measuredSNR, ...
                        realization,psf.FWHMRadialMM,psf.FWHMTangentialMM, ...
                ...
                fprintf('  %-18s | SNR %3g | rep %d/%d | +%d alg | %d/%d rows\n', ...
                    char(geometry),snrDb,realization,numReps, ...
                    numel(missingAlgorithms),height(results),expectedRows);
            end
        end
    end
end
...
if height(results) ~= expectedRows
    error('PACT:IncompleteResolutionRevision', ...
        'Expected %d revised rows, but obtained %d.',expectedRows,height(results));
end

finalCsv = fullfile(outputDir,'resolution_9point_results.csv');
finalMat = fullfile(outputDir,'resolution_9point_results.mat');
writetable(results,finalCsv);
save(finalMat,'results','cfg','-v7.3');

fid = fopen(fullfile(outputDir,'RESOLUTION_9POINT_RUN_MANIFEST.txt'),'w');
if fid>=0
    fprintf(fid,'PACT v9.7 9-point resolution completion\n');
    fprintf(fid,'Original resolution folder: %s\n',oldResolutionDir);
    fprintf(fid,'Configured points: 9\n');
    fprintf(fid,'SNR values: %s\n',mat2str(snrValues));
    fprintf(fid,'Realizations per SNR: %d\n',numReps);
    fprintf(fid,'Full-view algorithms: %s\n',strjoin(cellstr(fullAlgorithms),', '));
    fprintf(fid,'Limited-view algorithms: %s\n',strjoin(cellstr(limitedAlgorithms),', '));
    fprintf(fid,'Final rows: %d\n',height(results));
    fprintf(fid,'Full-ring adaptive-MV rows intentionally excluded from revised comparison.\n');
    fclose(fid);
end

fprintf('\n======================================================================\n');
fprintf(' 9-POINT RESOLUTION COMPLETION FINISHED\n');
...
%% ========================================================================
function tf = resolutionRowExistsV97(results,pointIndex,geometry, ...
...
%% ========================================================================
function results = uniqueResolutionRowsV97(results)
if isempty(results), return; end
keys = string(results.PointIndex)+"|"+results.SensorGeometry+"|"+ ...
    results.Algorithm+"|"+string(results.RequestedSNR_dB)+"|"+ ...
    string(results.NoiseRealization);

function PACT_SoundSpeedMismatch()
% PACT_BENCHMARK_V94_MISMATCH_ONLY
% Reviewer-focused rerun: acoustic sound-speed / delay-model mismatch only.
% Reuses the existing v9.3 output directory and cached forward data.
% Reproducible 2-D PACT benchmark for a methods paper.
%
% This version intentionally DOES NOT contain an iterative forward-adjoint
% reconstruction or an iterative-convergence study. It evaluates direct
% beamforming and adaptive array-processing methods only.
%
% Main additions relative to v8.0:
%   1) SNR-controlled repeated-noise experiments.
%   2) DASDSF and noise-corrected variance-driven DAS (NC-VDAS).
%   3) Noise-corrected coherence factor (NC-CF).
%   4) Conventional spatially smoothed MV (SS-MV) as a numerical reference.
%   5) Sequential Sherman-Morrison MV (SM-MV) using the SAME covariance
%      model and diagonal loading as SS-MV.
%   6) CF-, DASDSF-, and NC-VDAS-weighted SM-MV variants.
%   7) CNR, gCNR, target SNR, contrast, artifact power, and boundary metrics.
%   8) Point-spread-function tests for radial/tangential FWHM and PSL.
%   9) Numerical-equivalence and computational-complexity benchmarks for MV.
%  10) Parameter-sensitivity, paired statistical comparisons, rankings,
%      checkpoints, and manuscript-ready CSV/MAT/PNG outputs.
%
% IMPORTANT CLAIM BOUNDARIES:
%   - k-Wave is used for heterogeneous full-wave FORWARD simulation.
%   - Reconstruction is direct beamforming, not iterative model-based inversion.
%   - Independent image normalization means structural, not quantitative,
%     initial-pressure/optical-property recovery is evaluated.
%   - SM-MV removes an explicit L-by-L factorization for a fixed subarray
%     length L. A general O(M^3)-to-O(M) claim is not valid unless L is fixed.
%
% Required:
%   MATLAB R2020b or newer, k-Wave with kWaveArray, Image Processing Toolbox.
%
% Recommended workflow:
%   1) Use cfg.profile = "SMOKE_TEST" to verify the installation.
%   2) Use cfg.profile = "PAPER_CORE" while developing figures/tables.
%   3) Use cfg.profile = "PAPER_FULL" for the final manuscript experiment.
%
% Reviewed and rewritten: 2026-07-11

close all;
clc;

cfg = defaultConfiguration();
cfg = applyRunProfile(cfg, cfg.profile);
% Convert to an absolute path so later exports remain valid even if the
% current MATLAB folder changes during a long run.
if ~isAbsolutePathV91(cfg.outputDir)
    cfg.outputDir = fullfile(pwd, char(cfg.outputDir));
end
rng(cfg.masterSeed, 'twister');

assertRequiredFunctionsV90();
validateConfigurationV90(cfg);
prepareOutputDirectories(cfg);
writeRunManifest(cfg);

%% Grid and physical background
xVec = ((1:cfg.Nx) - (cfg.Nx + 1)/2) * cfg.dx;
yVec = ((1:cfg.Ny) - (cfg.Ny + 1)/2) * cfg.dy;
[Y, X] = meshgrid(yVec, xVec); % rows -> x, columns -> y
kgrid = kWaveGrid(cfg.Nx, cfg.dx, cfg.Ny, cfg.dy);
kgrid.setTime(cfg.Nt, cfg.dt);
tArray = (0:cfg.Nt-1) * cfg.dt;

background = buildClinicalBackground(X, Y, cfg);
roiMask = background.bodyMask;

[karray, sensorXFull, sensorYFull, limitedIndices] = ...
    makeFiniteApertureArrayV90(kgrid, cfg);

fprintf('Precomputing %s travel-time maps...\n', char(cfg.delayModel));
travelTimesFull = computeTravelTimes(X, Y, xVec, yVec, ...
    sensorXFull, sensorYFull, background.soundSpeed, cfg);

[phantomContrasts, phantomNames] = generateArticlePhantoms(X, Y);
selectedPhantomIndices = selectPhantomIndices(phantomNames, cfg.mainPhantomNames);

save(fullfile(cfg.outputDir, 'simulation_configuration.mat'), ...
    'cfg', 'background', 'sensorXFull', 'sensorYFull', ...
    'limitedIndices', 'phantomNames', '-v7.3');
writeConfigurationJSON(cfg);
saveBackgroundFigure(background, xVec, yVec, cfg.outputDir);

fprintf('======================================================================\n');
fprintf(' PACT BENCHMARK v9.3 - REVIEWER-REVISION DIRECT RECONSTRUCTION\n');
fprintf(' Profile: %s | Phantoms: %d | SNR levels: %d\n', ...
    char(cfg.profile), numel(selectedPhantomIndices), numel(cfg.snrDb));
fprintf(' No iterative reconstruction or iterative-convergence claims are used.\n');
fprintf('======================================================================\n');

allResults = emptyMainResultsTable();
allEquivalence = emptyEquivalenceTable();
allSensitivity = emptySensitivityTable();

for pCounter = 1:numel(selectedPhantomIndices)
    p = selectedPhantomIndices(pCounter);
    phantomName = phantomNames(p);
    contrast = phantomContrasts{p};

    phantomDir = fullfile(cfg.outputDir, char(phantomName));
    imageDir = fullfile(phantomDir, 'reconstruction_images');
    dataDir = fullfile(phantomDir, 'cached_forward_data');
    ensureDirectory(phantomDir);
    ...
    [targetMask, backgroundMask] = makeEvaluationMasks( ...
        gtNorm, roiMask, cfg.segmentationThreshold, ...
        cfg.backgroundGtMaximum, cfg.backgroundMarginMM, cfg.dx);

    savePhysicalMaps(optical, background, xVec, yVec, phantomDir, phantomName);
    save(fullfile(phantomDir, 'evaluation_masks.mat'), ...
        'roiMask', 'targetMask', 'backgroundMask', 'gtNorm', '-v7.3');

    cleanDataFile = fullfile(dataDir, 'clean_full_ring_data.mat');
    reuseFile = "";
    if strlength(string(cfg.reuseForwardCacheRoot))>0
        reuseFile = fullfile(char(cfg.reuseForwardCacheRoot),char(phantomName), ...
            'cached_forward_data','clean_full_ring_data.mat');
    end
    if cfg.resume && isfile(cleanDataFile)
        loaded = load(cleanDataFile, 'cleanFullData');
        cleanFullData = loaded.cleanFullData;
        fprintf('\n[%d/%d] %s: loaded cached forward data.\n', ...
            pCounter, numel(selectedPhantomIndices), char(phantomName));
    elseif strlength(string(reuseFile))>0 && isfile(reuseFile)
        loaded = load(reuseFile,'cleanFullData');
        cleanFullData = loaded.cleanFullData;
        save(cleanDataFile,'cleanFullData','-v7.3');
        fprintf('\n[%d/%d] %s: reused prior cached forward data.\n', ...
            pCounter,numel(selectedPhantomIndices),char(phantomName));
    else
        fprintf('\n[%d/%d] %s: running heterogeneous k-Wave forward model...\n', ...
            pCounter, numel(selectedPhantomIndices), char(phantomName));
        cleanFullData = simulateKWaveFiniteArray(gtP0, background, ...
            kgrid, karray, cfg);
        cleanFullData = single(cleanFullData);
        save(cleanDataFile, 'cleanFullData', '-v7.3');
    end

    checkpointFile = fullfile(phantomDir, 'results_checkpoint.mat');
    if cfg.resume && isfile(checkpointFile)
        loaded = load(checkpointFile, 'results', 'equivalenceResults', ...
            'sensitivityResults');
        results = loaded.results;
        if isfield(loaded, 'equivalenceResults')
            equivalenceResults = loaded.equivalenceResults;
        else
            equivalenceResults = emptyEquivalenceTable();
        end
        if isfield(loaded, 'sensitivityResults')
            sensitivityResults = loaded.sensitivityResults;
        else
            sensitivityResults = emptySensitivityTable();
        end
    else
        results = emptyMainResultsTable();
        equivalenceResults = emptyEquivalenceTable();
        sensitivityResults = emptySensitivityTable();
    end

    for geometry = cfg.sensorGeometries
        [sensorIndices, isCircularArray] = geometryIndices( ...
            geometry, cfg, limitedIndices);
        ...
        fprintf('  Geometry: %-18s | elements: %d\n', ...
            char(geometry), numSensors);

        for snrIndex = 1:numel(cfg.snrDb)
            requestedSNR = cfg.snrDb(snrIndex);
            numRealizations = realizationCountForSNR(requestedSNR, cfg);

            for realization = 1:numRealizations
                algorithms = algorithmsForCase(phantomName, requestedSNR, realization, cfg);
                % In a circular full ring, cyclic subarray averaging makes the
                % current SM-MV family algebraically duplicate DAS and its
                % corresponding coherence-weighted variants. Do not treat these
                % degenerate full-ring adaptive rows as independent competitors.
                if isCircularArray
                    algorithms = setdiff(algorithms,cfg.adaptiveAlgorithms,'stable');
                end
                directMVRequired = isDirectMVValidationCase( ...
                    phantomName, geometry, requestedSNR, realization, cfg);
                if directMVRequired
                    algorithms = unique([algorithms, "SM-MV", "SS-MV"], 'stable');
                end

                if allMainResultsExist(results, phantomName, geometry, ...
                        requestedSNR, realization, algorithms)
                    continue;
                end

                rng(caseSeed(cfg.masterSeed, p, geometry, requestedSNR, realization), ...
                    'twister');
                [noisyData, knownNoiseSigma, measuredSNR] = ...
                    addNoiseAtRequestedSNR(cleanData, requestedSNR);
                ...
                context.isCircularArray = isCircularArray;
                ...
                montageEntries = struct('Algorithm', {}, 'Image', {}, 'Metrics', {});
                smRawImage = [];
                directRawImage = [];
                smRuntime = NaN;
                directRuntime = NaN;

                % Reviewer-driven amplitude analysis: use one DAS-derived global
                % calibration factor as a common scaling rule for all algorithms in
                % this phantom/geometry/SNR/noise-realization case. In addition,
                % each algorithm is evaluated with its own optimal zero-intercept
                % global linear scale, as requested by Reviewer 2.
                commonDASScale = NaN;
                dasIndex = find(string({outputs.Algorithm}) == "DAS", 1);
                if ~isempty(dasIndex)
                    commonDASScale = optimalGlobalScale(outputs(dasIndex).RawImage, ...
                        gtP0, roiMask);
                end
                ...
                for a = 1:numel(outputs)
                    ...
                    if algorithm == "SM-MV"
                        smRawImage = outputs(a).RawImage;
                        smRuntime = outputs(a).BeamformRuntimeSec;
                    elseif algorithm == "SS-MV"
                        directRawImage = outputs(a).RawImage;
                        directRuntime = outputs(a).BeamformRuntimeSec;
                    end
                    if mainResultExists(results, phantomName, geometry, ...
                            requestedSNR, realization, algorithm)
                        continue;
                    end
                    ...
                    metrics = computeArticleMetrics(recNorm, gtNorm, roiMask, ...
                        ...
                        cfg.dx, cfg.gCNRBins);
                    amplitudeMetrics = computeAmplitudeFidelityMetrics( ...
                        outputs(a).RawImage, gtP0, roiMask, targetMask, X, Y, ...
                        cfg.bodyRadius, commonDASScale);

                    totalRuntime = delayRuntime + outputs(a).BeamformRuntimeSec;
                    row = makeMainResultRow(phantomName, geometry, algorithm, ...
                        requestedSNR, measuredSNR, realization, knownNoiseSigma, ...
                        estimatedNoiseSigma, metrics, amplitudeMetrics, delayRuntime, ...
                        outputs(a).BeamformRuntimeSec, totalRuntime, numSensors, ...
                        cfg, gtP0, optical, targetMask, backgroundMask);
                    results = [results; row]; %#ok<AGROW>

                    fprintf(['    %-18s | SNR %7s dB | rep %02d | ' ...
                        'PSNR %6.2f | SSIM %.4f | gCNR %.3f | HD95 %.3f mm\n'], ...
                        char(algorithm), formatSNR(requestedSNR), realization, ...
                        metrics.PSNR, metrics.SSIM, metrics.gCNR, metrics.HD95MM);

                    if shouldSaveReconstruction(requestedSNR, realization, cfg)
                        montageEntries(end+1).Algorithm = algorithm; %#ok<AGROW>
                        montageEntries(end).Image = recNorm;
                        montageEntries(end).Metrics = metrics;
                        saveReconstructionArray(recNorm, outputs(a).RawImage, ...
                            phantomName, geometry, algorithm, requestedSNR, ...
                            realization, imageDir, cfg);
                    end
                end

                if directMVRequired && ~isempty(smRawImage) && ~isempty(directRawImage)
                    eqRow = compareMVImages(phantomName, geometry, requestedSNR, ...
                        realization, directRawImage, smRawImage, directRuntime, smRuntime, ...
                        cfg.mvSubarrayLength, cfg.mvDiagonalLoading);
                    equivalenceResults = [equivalenceResults; eqRow]; %#ok<AGROW>
                end

                if shouldRunSensitivityCase(phantomName, geometry, ...
                        requestedSNR, realization, cfg)
                    newSensitivity = runParameterSensitivity(delayed, gtNorm, ...
                        roiMask, targetMask, backgroundMask, context, cfg, ...
                        phantomName, geometry, requestedSNR, realization);
                    sensitivityResults = [sensitivityResults; newSensitivity]; %#ok<AGROW>
                    sensitivityResults = uniqueSensitivityResults(sensitivityResults);
                end

                if ~isempty(montageEntries)
                    saveCaseMontage(gtNorm, montageEntries, xVec, yVec, ...
                        phantomName, geometry, requestedSNR, realization, imageDir);
                end

                save(checkpointFile, 'results', 'equivalenceResults', ...
                    'sensitivityResults', '-v7.3');
                writetable(results, fullfile(phantomDir, 'results.csv'));
                writetable(equivalenceResults, ...
                    fullfile(phantomDir, 'mv_image_equivalence.csv'));
                writetable(sensitivityResults, ...
                    fullfile(phantomDir, 'parameter_sensitivity.csv'));
            end
        end
    end

    allResults = [allResults; results]; %#ok<AGROW>
    allEquivalence = [allEquivalence; equivalenceResults]; %#ok<AGROW>
    allSensitivity = [allSensitivity; sensitivityResults]; %#ok<AGROW>

    % Save a master checkpoint after every completed phantom.
    saveCoreResultsCheckpointV91(cfg.outputDir, allResults, ...
        allEquivalence, allSensitivity, cfg);
end

%% Persist the main reconstruction study BEFORE optional benchmarks
...
saveCoreResultsCheckpointV91(cfg.outputDir, allResults, ...
    allEquivalence, allSensitivity, cfg);
publishResultsToBaseWorkspaceV91(cfg, allResults, allEquivalence, ...
    allSensitivity, table(), table(), table());

%% Dedicated resolution benchmark (failure-safe)
resolutionResults = table();
if cfg.runResolutionBenchmark
    try
        resolutionResults = runResolutionBenchmark(background, kgrid, karray, ...
            travelTimesFull, limitedIndices, X, Y, xVec, yVec, tArray, roiMask, cfg);
        if ~isempty(resolutionResults)
            writetable(resolutionResults, ...
                fullfile(cfg.outputDir, 'resolution_results.csv'));
            save(fullfile(cfg.outputDir, 'resolution_results.mat'), ...
                'resolutionResults', '-v7.3');
        end
    catch ME
        saveBenchmarkFailureV91(cfg.outputDir, 'resolution', ME);
        warning('PACT:ResolutionBenchmarkFailed', ...
            ['Resolution benchmark failed, but core reconstruction results ' ...
             'were preserved. Reason: %s'], ME.message);
    end
end

%% Reviewer-driven acoustic model-mismatch benchmark (failure-safe)
delayMismatchResults = table();
if cfg.runDelayMismatchBenchmark
    try
        delayMismatchResults = runDelayMismatchBenchmark(background, ...
            phantomContrasts, phantomNames, selectedPhantomIndices, ...
            sensorXFull, sensorYFull, limitedIndices, X, Y, xVec, yVec, ...
            tArray, roiMask, cfg);
        if ~isempty(delayMismatchResults)
            writetable(delayMismatchResults, ...
                fullfile(cfg.outputDir,'delay_mismatch_results.csv'));
            save(fullfile(cfg.outputDir,'delay_mismatch_results.mat'), ...
                'delayMismatchResults','-v7.3');
        end
    catch ME
        saveBenchmarkFailureV91(cfg.outputDir,'delay_mismatch',ME);
        warning('PACT:DelayMismatchBenchmarkFailed', ...
            'Delay-mismatch benchmark failed but core results were preserved: %s',ME.message);
    end
end

%% Reviewer-driven noise robustness benchmark (failure-safe)
noiseRobustnessResults = table();
if cfg.runNoiseRobustnessBenchmark
    try
        noiseRobustnessResults = runNoiseRobustnessBenchmark(background, ...
            phantomContrasts, phantomNames, selectedPhantomIndices, ...
            limitedIndices, X, Y, tArray, roiMask, travelTimesFull, cfg);
        if ~isempty(noiseRobustnessResults)
            writetable(noiseRobustnessResults, ...
                fullfile(cfg.outputDir,'noise_robustness_results.csv'));
            save(fullfile(cfg.outputDir,'noise_robustness_results.mat'), ...
                'noiseRobustnessResults','-v7.3');
        end
    catch ME
        saveBenchmarkFailureV91(cfg.outputDir,'noise_robustness',ME);
        warning('PACT:NoiseRobustnessBenchmarkFailed', ...
            'Noise robustness benchmark failed but core results were preserved: %s',ME.message);
    end
end

%% MV equivalence and complexity benchmark (failure-safe)
complexityResults = table();
complexitySlopes = table();
syntheticEquivalence = table();
if cfg.runComplexityBenchmark
    try
        [complexityResults, complexitySlopes, syntheticEquivalence] = ...
            runMVComplexityBenchmark(cfg);
        if ~isempty(complexityResults)
            writetable(complexityResults, ...
                fullfile(cfg.outputDir, 'mv_complexity_results.csv'));
            writetable(complexitySlopes, ...
                fullfile(cfg.outputDir, 'mv_complexity_slopes.csv'));
            writetable(syntheticEquivalence, ...
                fullfile(cfg.outputDir, 'mv_synthetic_equivalence.csv'));
        end
    catch ME
        saveBenchmarkFailureV91(cfg.outputDir, 'complexity', ME);
        warning('PACT:ComplexityBenchmarkFailed', ...
            ['Complexity benchmark failed, but core reconstruction results ' ...
             'were preserved. Reason: %s'], ME.message);
    end
end

%% Aggregate statistics and manuscript outputs
allResults = uniqueMainResults(allResults);
allEquivalence = uniqueEquivalenceResults(allEquivalence);
allSensitivity = uniqueSensitivityResults(allSensitivity);

writetable(allResults, fullfile(cfg.outputDir, 'all_reconstruction_results.csv'));
save(fullfile(cfg.outputDir, 'all_reconstruction_results.mat'), ...
    'allResults', '-v7.3');

summaryTable = makeArticleSummary(allResults, cfg);
pairedTable = makePairedComparisons(allResults, cfg);
rankingTable = makeAlgorithmRankings(summaryTable);
bestTable = makeBestAlgorithmTable(summaryTable);

writetable(summaryTable, fullfile(cfg.outputDir, 'statistical_summary.csv'));
writetable(pairedTable, fullfile(cfg.outputDir, 'paired_comparisons_vs_DAS.csv'));
writetable(rankingTable, fullfile(cfg.outputDir, 'algorithm_rankings.csv'));
writetable(bestTable, fullfile(cfg.outputDir, 'metric_leaders_by_condition.csv'));
writetable(allEquivalence, fullfile(cfg.outputDir, 'mv_image_equivalence.csv'));
writetable(allSensitivity, fullfile(cfg.outputDir, 'parameter_sensitivity.csv'));

paperTableDir = fullfile(cfg.outputDir, 'paper_tables');
writetable(summaryTable, fullfile(paperTableDir, 'Table_statistical_summary.csv'));
writetable(pairedTable, fullfile(paperTableDir, 'Table_paired_comparisons.csv'));
writetable(rankingTable, fullfile(paperTableDir, 'Table_algorithm_rankings.csv'));
writetable(bestTable, fullfile(paperTableDir, 'Table_metric_leaders.csv'));
writetable(allEquivalence, fullfile(paperTableDir, 'Table_MV_equivalence.csv'));

if ~isempty(resolutionResults)
    writetable(resolutionResults, fullfile(cfg.outputDir, 'resolution_results.csv'));
end
if ~isempty(complexityResults)
    writetable(complexityResults, fullfile(cfg.outputDir, 'mv_complexity_results.csv'));
    writetable(complexitySlopes, fullfile(cfg.outputDir, 'mv_complexity_slopes.csv'));
    writetable(syntheticEquivalence, ...
        fullfile(cfg.outputDir, 'mv_synthetic_equivalence.csv'));
end

saveSummaryFigures(summaryTable, cfg.outputDir);
if ~isempty(complexityResults)
    saveComplexityFigures(complexityResults, complexitySlopes, cfg.outputDir);
end
writeOutputDictionary(cfg.outputDir);
publishResultsToBaseWorkspaceV91(cfg, allResults, allEquivalence, ...
    allSensitivity, resolutionResults, complexityResults, complexitySlopes);

fprintf('\n======================================================================\n');
fprintf('PACT v9.3 reviewer-revision benchmark completed.\n');
fprintf('Results: %s\n', cfg.outputDir);
fprintf('Use the generated output dictionary before preparing manuscript tables.\n');
fprintf('======================================================================\n');
end
...
cfg.runResolutionBenchmark = false; % v9.4 mismatch-only rerun
...
cfg.runComplexityBenchmark = false; % v9.4 mismatch-only rerun
...
cfg.runParameterSensitivity = false; % v9.4 mismatch-only rerun
...
cfg.mismatchPhantomNames = ["Circle","Ring","Two_Circles","Ellipse", ...
    "Rectangle","Triangle","Shepp_Logan","Complex","Medical", ...
    "Letter_A","Vascular_Tree"];
cfg.mismatchGeometries = ["CIRCULAR_FULL","LIMITED_VIEW_ARC"];
cfg.mismatchSNRdB = [20,0];
cfg.mismatchRealizations = 3;
cfg.mismatchAlgorithms = ["DAS","CF-DAS","SCF-DAS","DMAS","DASDSF", ...
    "NC-CF","NC-VDAS","SM-MV","CF-SM-MV", ...
    "DASDSF-SM-MV","NCVDAS-SM-MV"];
...
cfg.runNoiseRobustnessBenchmark = false; % v9.4 mismatch-only rerun
...
cfg.outputDir = 'benchmark_v93_reviewer_revision'; % MUST point to existing v9.3 run folder
end
...
ensureDirectory(outputDir);
partialMat = fullfile(outputDir,'delay_mismatch_results_PARTIAL.mat');
partialCsv = fullfile(outputDir,'delay_mismatch_results_PARTIAL.csv');

% Resume safely from a previous mismatch-only run.
if cfg.resume && isfile(partialMat)
    loadedPartial = load(partialMat,'results');
    if isfield(loadedPartial,'results')
        results = loadedPartial.results;
    else
...
results = uniqueDelayMismatchResults(results);

% The baseline delay field uses the known heterogeneous sound-speed map but
% still uses straight-path slowness integration. It is therefore a
% KNOWN-MAP STRAIGHT-RAY reference, not a full-wave or refractive ground
% truth. All reported delay perturbations below are relative to this
% reference.
baselineCfg = cfg;
baselineCfg.delayModel = "STRAIGHT_RAY_HETEROGENEOUS";
baselineTravelTimesAll = computeTravelTimes(X,Y,xVec,yVec,sensorXFull, ...
    sensorYFull,background.soundSpeed,baselineCfg);
...
for requestedName = cfg.mismatchPhantomNames
    ...
    for condition = cfg.mismatchConditions
        ...
        % Quantify the magnitude of the assumed sound-speed-map error inside
        % the tissue ROI. These are map errors relative to the simulation
        % background, not estimates of experimental uncertainty.
        trueVals = double(background.soundSpeed(roiMask));
        assumedVals = double(assumedMap(roiMask));
        mapDiff = assumedVals-trueVals;
        mapMAE = mean(abs(mapDiff),'omitnan');
        mapRMSE = sqrt(mean(mapDiff.^2,'omitnan'));
        mapBias = mean(mapDiff,'omitnan');

        for geometry = cfg.mismatchGeometries
            [sensorIndices,isCircular] = geometryIndices(geometry,cfg,limitedIndices);
            cleanData = cleanFullData(sensorIndices,:);
            travelTimes = travelTimesAll(sensorIndices,:);
            baselineTravelTimes = baselineTravelTimesAll(sensorIndices,:);

            roiVector = roiMask(:).';
            delayDifference = double(travelTimes(:,roiVector)) - ...
                double(baselineTravelTimes(:,roiVector));
            delayRMSEns = 1e9*sqrt(mean(delayDifference(:).^2,'omitnan'));
            absDelayNs = 1e9*abs(delayDifference(:));
            delayP95ns = percentileFinite(absDelayNs,95);
            ...
            for snrDb = cfg.mismatchSNRdB
                ...
                for realization = 1:reps
                    ...
                    % Full-ring SM-MV is structurally degenerate to DAS for
                    % the present cyclic subarray averaging; adaptive MV
                    % variants are therefore excluded from full-ring mismatch
                    % ranking and evaluated in the non-degenerate limited view.
                    if isCircular
                        algorithms = algorithms(~contains(algorithms,"SM-MV"));
                    end

                    missing = strings(0,1);
                    for aa = 1:numel(algorithms)
                        if ~delayMismatchResultExists(results,phantomName,condition, ...
                                geometry,snrDb,realization,algorithms(aa))
                            missing(end+1,1) = algorithms(aa); %#ok<AGROW>
                        end
                    end
                    if isempty(missing)
                        continue;
                    ...
                    % The seed deliberately does not depend on mismatch
                    % condition, so every sound-speed condition receives the
                    % same noise realization for a paired comparison.
                    ...
                    context.noiseVariance = noiseVar;
                    context.numSensors = numel(sensorIndices);
                    outputs = reconstructAlgorithmSet(delayed,missing,cfg,context);
                    for a = 1:numel(outputs)
                        recNorm = normalizeAmplitude(outputs(a).RawImage,roiMask);
                        metrics = computeArticleMetrics(recNorm,gtNorm,roiMask, ...
                            targetMask,backgroundMask,cfg.segmentationThreshold, ...
                        ...
                        row(1,:) = {string(phantomName),string(condition), ...
                            ...
                            measuredSNR,realization,mapMAE,mapRMSE,mapBias, ...
                            delayRMSEns,delayP95ns,metrics.PSNR,metrics.SSIM, ...
        ...
        % Crash-safe mismatch checkpoint after every completed condition.
        results = uniqueDelayMismatchResults(results);
        save(partialMat,'results','-v7.3');
        writetable(results,partialCsv);
        fprintf('Mismatch checkpoint: %-18s | %-22s | rows %d\n', ...
            char(phantomName),char(condition),height(results));
    end
end
results = uniqueDelayMismatchResults(results);
end
...
names = {'Phantom','MismatchCondition','SensorGeometry','Algorithm', ...
    'RequestedSNR_dB','MeasuredSNR_dB','NoiseRealization', ...
    'SoundSpeedMAE_mps','SoundSpeedRMSE_mps','SoundSpeedBias_mps', ...
    'DelayRMSE_vsKnownMap_ns','DelayP95Abs_vsKnownMap_ns', ...
    'PSNR_ROI_dB','SSIM','Dice','HD95_mm','gCNR','ArtifactToTarget_dB'};
types = [{'string','string','string','string'},repmat({'double'},1,14)];
tableOut = table('Size',[0,numel(names)],'VariableTypes',types, ...
    'VariableNames',names);
end

%% ========================================================================
function tf = delayMismatchResultExists(results,phantom,condition,geometry, ...
...
tf = any(results.Phantom==string(phantom) & ...
    results.MismatchCondition==string(condition) & ...
    results.SensorGeometry==string(geometry) & snrMask & ...
    results.NoiseRealization==realization & ...
    results.Algorithm==string(algorithm));
end

%% ========================================================================
function results = uniqueDelayMismatchResults(results)
if isempty(results), return; end
keys = table(results.Phantom,results.MismatchCondition,results.SensorGeometry, ...
    results.Algorithm,results.RequestedSNR_dB,results.NoiseRealization, ...
    'VariableNames',{'Phantom','MismatchCondition','SensorGeometry', ...
    'Algorithm','RequestedSNR_dB','NoiseRealization'});
...
switch condition
    case "BASELINE_HETERO"
        % Known-map straight-ray reference; not a refractive ground truth.
    ...
    case "UNDERSEGMENT_2PX"
        % Synthetic segmentation error: shrink heterogeneous tissue regions
        % by the requested number of pixels while preserving the coupling
        % medium outside the body. Pixels lost by under-segmentation are
        % assigned the nominal homogeneous tissue speed.
        c0 = cfg.homogeneousReconSoundSpeed;
        bodyMask = hypot(X,Y) <= cfg.bodyRadius;
        heteroMask = bodyMask & abs(double(trueMap)-c0) > 5;
        radius = max(1,round(cfg.mismatchSegmentationPixels));
        eroded = imerode(heteroMask,strel('disk',radius,0));
        lostPixels = heteroMask & ~eroded;
        assumedMap = double(trueMap);
        assumedMap(lostPixels) = c0;
    otherwise
